...
if (isset($_POST['sub'])) {
    ...
    $result = $connect->query("select * from infrastructure_details where PHCid=1");

    if ($result->num_rows >0) {
        $update_query = $connect->query("update infrastructure_details set no_labs='$no_labs',no_beds='$no_beds',labour_room='$labour_room',waiting_circulatory_space='$waiting_room',opd_room='$opd_room',wards='$ward_room',operation_theater='$operation_theater',strechers='$strechers',emergency_entry_exit='$emergency',pharmacy='$pharmacy',maternity_room='$maternity_room',doctor_room='$doctor_room',visiting_doctor_room='$visit_doctor_room',public_utilities='$public_utilities',electricity_availability='$electricity',registration_counter='$registration_counter' where PHCid=1");
        echo "<script>alert('Inserted Successfully')</script>";
        header("Location: /infrastructure.php");
        exit();

    } else {
        $insert_query = $connect->query("insert into infrastructure_details values ('1','$no_labs','$no_beds','$labour_room','$waiting_room','$opd_room','$ward_room','$operation_theater','$strechers','$emergency','$pharmacy','$maternity_room','$doctor_room','$visit_doctor_room','$public_utilities','$electricity','$registration_counter')");
